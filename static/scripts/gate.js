...
SocketDraw = {none:0, circle:1, square: 2};

var GateDefaults = {
	socket: SocketDraw.square
};
...
function Gate(gateType, x, y, lbl, dply, noInit)
{
	...
	this.render = function(context, offset, selectClr)
	{
		...
		for (var i = 0; i < this.inputs.length + this.outputs.length; ++ i) {
			...
			context.beginPath();
			switch(GateDefaults.socket){
				case SocketDraw.circle: 
					context.arc(pos.x + offset.x, pos.y + offset.y, 4, 0, Math.PI * 2, true);
					context.fill();
					context.stroke();
					break;
				case SocketDraw.square: 
					//context.fillStyle = selectClr;
					context.rect(pos.x + offset.x-4, pos.y + offset.y-4, 8, 8);
					context.fill();
					context.stroke();
					break;
			}
