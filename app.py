...
def simplificar_algebra():
    ...
    try:
        local_dict = {chr(i): symbols(chr(i)) for i in range(65, 91)}
        for i in range(10):
            local_dict[str(i)] = symbols(str(i))

        expr = parse_expr(sympy_str, local_dict=local_dict)
        pasos = []
        current_expr = expr

        # 1. Teoremas de De Morgan
        nnf_expr = to_nnf(current_expr, simplify=False)
        if str(nnf_expr) != str(current_expr):
            pasos.append({
                "regla": "Leyes de De Morgan y Doble Negación:",
                "ecuacion_previa": str(current_expr),
                "ecuacion": str(nnf_expr)
            })
            current_expr = nnf_expr

        # 2. Expansión / Ley Distributiva (MULTIPLICAR TÉRMINOS)
        dnf_expr = to_dnf(current_expr, simplify=True)
        if str(dnf_expr) != str(current_expr):
            pasos.append({
                "regla": "Ley Distributiva (Expandiendo términos):",
                "ecuacion_previa": str(current_expr),
                "ecuacion": str(dnf_expr)
            })
            current_expr = dnf_expr

        # 3. Simplificación Final (Absorción, Idempotencia)
        final_expr = simplify_logic(current_expr, force=True)
        if str(final_expr) != str(current_expr):
            pasos.append({
                "regla": "Leyes de Absorción, Idempotencia y Complemento:",
                "ecuacion_previa": str(current_expr),
                "ecuacion": str(final_expr)
            })
            current_expr = final_expr

        # Si no hubo nada que hacer
        if len(pasos) == 0:
            pasos.append({
                "regla": "La expresión ya se encuentra en su forma más simple:",
                "ecuacion_previa": str(expr),
                "ecuacion": str(expr)
            })
        ...
        return jsonify({
            'exito': True,
            'original': str(expr),
            'final': str(current_expr),
            'pasos': pasos
        })
    except Exception as e:
        return jsonify({'exito': False, 'error': "Sintaxis inválida. Por favor revisa los paréntesis y operadores."})
